...
io.on('connection', (socket) => {
    ...
    socket.on('newUserName', (msg) => {
        ...
        // onlineUser 数组中id相同的用户去重
        // 将用户名称返回在线玩家列表
        socket.emit('listOnlineUserResponse', { status: 0, onlineUser: onlineUser });
        // 通知所有在线用户更新在线用户列表
        io.emit('listOnlineUserResponse', { status: 0, onlineUser: onlineUser });
    })
    socket.on('listOnlineUserResponse', (msg) => {
        console.log('listOnlineUserResponse', msg);
    })
    // 用户点击 开始游戏, 触发对手匹配
    ...
    function startGameCallBack(msg) {
        ...
        deal.then((result) => {
            ...
            // 修改onlineUser状态
            onlineUser = onlineUser.map((item) => {
                if (item.id == socket.id || item.id == result) {
                    item.status = 2;
                }
                return item;
            })
    ...
    socket.on('chess', (msg) => {
        ...
        if (!msg.isWin) {
            ...
            // 修改onlineUser状态
            onlineUser = onlineUser.map((item) => {
                if (item.id == socket.id || item.id == msg.againstId) {
                    item.status = 0;
                }
                return item;
            })
    ...
    socket.on('accident', (msg) => {
        ...
        if (msg.status == 1) {        // 表示发送该消息方 认输
            ...
            // 修改onlineUser状态
            onlineUser = onlineUser.map((item) => {
                if (item.id == socket.id || item.id == socketList[socket.id].opponent) {
                    item.status = 0;
                }
                return item;
            }
